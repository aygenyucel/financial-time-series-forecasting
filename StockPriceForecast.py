...
end = dt.datetime(2020,7,24)      #(year, month, date)

data = web.DataReader(company, 'yahoo', start-dt.timedelta(days=365), end)
...
#Load Test Data
test_start = dt.datetime(2019,7,24)
test_end = dt.datetime(2020,7,24)
...
print(f"Date: {tomorrow}: {company} CLOSE Price Forecasting: {prediction}") # Stock
...
plt.figure(figsize=(12,5), dpi=100)
plt.plot(total_dataset, color = 'orange', label = f"Training {company} Close Price DATA")
plt.plot(test_data['Close'], color = 'green', label = f"Predicted {company} Close Price DATA")
plt.title(f"{company} Prediction Process")
plt.xlabel('Time')
